...
{
    /**
     * @Route("/my-tasks/{page}/{state}", name="my_tasks",defaults={"page"=1, "state"=0})
     */
    public function index(Request $request, $page, $state)
    {
        $clientId = $this->get('security.token_storage')->getToken()->getUser()->getId();
        $entityManager = $this->getDoctrine()->getManager();
        if ($state == 0) { // Ver todos
            $tasks = $entityManager->getRepository("App\Entity\Task")->findBy(['client' => $clientId]);
